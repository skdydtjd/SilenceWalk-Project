...
Unity6 (6000.0.32f1), URP, Windows, C#, 3D
...
Ex) 파이프 퍼즐 부분, 카메라 시야 처리, 적 행동로직 뼈대 마련 등
...
이 때 NavSurface의 연결이 끊어진 지형의 경우 NavLink를 연결하여 해결했으나 NavMesh의 특성상 다익스트라 알고리즘을 기반으로 되어있기 때문에 배회 중 플레이어를 추적하는 상태로 바뀌면 배회 당시 Link로 연결되어있는 지점을 거쳐가는 것을 최적 경로로 판단하여 추적 도중 Link를 타고 들어갔다 나오는 버벅이는 현상이 발생했다.
...
(AI 툴(ChatGPT, Gemini 등)을 활용하여 코드를 재 검증 중 NevMesh의 autoTraverseOffMeshLink라는 근처에 NevLink가 존재할 경우 경로 계산시 자동으로 권한을 가지는 요소를 확인)

(강제적으로 Link를 타게하는 autoTraverseOffMeshLink를 CheckOFF하고 수동적으로 이동할 수 있도록 AI툴(ChatGPT, Gemini 등)을 활용한 코루틴 코드를 BazicEnemyAI에 추가)

(최종적으로 들낙거리는 현상을 없애고 자연스럽게 쫓아오도록 완화하였음)
...
Editor에서는 실행에 문제가 없었지만 빌드가 안되는 문제가 발생하여 에러도 아닌 처음보는 경고로 빌드문제에 직면하여 고생을 좀 겪었다.
...
이를 통해 많은 광원이 있을 경우 필요에 따라 게임 진행에 방해되지않는 선에서 텍스쳐에 그림자를 적절히 배치, 생성해야할 필요가 있음을 느꼈다.

-----------------------------------------------------------------------------------------------------

__3. 저장 후 불러오기를 진행할 떄 빌드가 완전히 이루어지기전 불러오기가 이루어져 일부 데이터가 유실되는 문제__

각 스테이지 클리어 판정 여부를 저장하고 불러올 때 bool형의 경우 인게임 적용에 문제가 없었으나 int형으로 판단하는 스테이지의 경우 해제되어 있어야 할 문이 그대로 잠겨있는 상황과 같이 일부 데이터를 제대로 읽어들이지 못하는 문제가 발생했다.

데이터를 불러올 때 게임 내 사전 빌드가 완전히 이루어진 뒤 불러올 수 있도록하는 코드를 추가 해줌으로서 문제를 해결했다.
또한 저장되어있던 값으로 다시 한번 클리어 조건 검사를 진행해주는 코드를 리로드되는 과정 중에 이뤄질 수 있게끔 추가했으며 각 문(Door)의 동작을 담당하는 스크립트의 Start() 구문 안에도 검사구문을 추가함으로서 안전하게 값을 받을 수 있도록 했다.

이를 통해 불러오는 과정에서 조건을 다시 한번 확인을 해주는 절차가 필요함을 알게되었으며 안전하게 씬이 빌드된 후 게임 내 데이터를 불러와야 유실을 방지 할수 있음을 배울 수 있었다.

또한 FrameWork 구성의 필요성을 강하게 느끼고 추후 SceneManager, ResourceManager, DataManager 등의 FrameWork를 미래의 프로젝트에 적극 활용할 것을 다짐했다.
...
이 과정 중 에셋이 카메라에 보이는 시야 범위에 따라 렌더링이 뚜렷하게 또는 흐리게, 컬링이 구현되는 LOD Group이란 것을 사용하고 있어 기본적인 Material과 다르게 구현되어 있는 것을 처음 알게 되었고 이를 해결하기위해 foreach문을 중복 사용하여 각 LOD Group 내에 있는 각 Material에 접근이 가능하도록 하여 새로 만든 반투명 Meterial로 교체했다가 사이 공간에 있는 물체가 사라지면 다시 원상복구가 되도록 구현해 보았다.

# 플레이 링크

(The First Version (v1))

https://play.unity.com/en/games/5c1df5d8-919b-4d8d-9caf-79b17480924c/silencewalk-demo-version-v1
...
https://play.unity.com/ko/games/e57f9968-5a69-4889-9dbd-1efa261508c7/silencewalk-20260524-v3 (현재 OOM(Out of Memory) 오류가 있음)
