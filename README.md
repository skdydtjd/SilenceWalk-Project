...
또한 저장되어있던 값으로 다시 한번 클리어 조건을 검사를 진행해주는 코드를 게임이 리로드되는 과정 중에 이뤄지게끔 추가했으며 각 문(Door)의 동작을 담당하는 스크립트의 Start() 구문 안에도 검사가 되게 함으로서 안전하게 값을 받을 수 있도록 했다.

이를 통해 불러오는 과정에서 조건을 다시 한번 확인을 해주는 절차가 필요함을 알게되었으며 안전하게 씬이 빌드된 후 게임 내 데이터를 불러와야 손실이 되지않음을 배울 수 있었다.
