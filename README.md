...
__3. 저장 후 불러오기를 진행할 떄 빌드가 완전히 이루어지기전 불러오기가 이루어져 일부 데이터가 손실되는 문제__

각 스테이지 클리어 판정 여부를 저장할 때 bool형의 경우 불러온 뒤 인게임 적용에 문제가 없었으나 int형으로 판단하는 스테이지의 경우 데이터를 불러온 뒤 해제되어 있어야 할 문이 그대로 잠겨있는 상황이 발생했다.

데이터를 불러올 때 게임 내 사전 빌드가 완전히 이루어진 뒤 불러올 수 있도록하는 코드를 추가 해줌으로서 문제를 해결했다.
또한 저장되어있던 값으로 다시 한번 클리어 조건을 검사를 진행해주는 코드를 게임이 리로드되는 과정 중에 이뤄지게끔 추가했으며 각 문(Door)의 동작을 담당하는 스크립트의 Start() 구문 안에도 2중으로 검사가 되게 함으로서 안전하게 값을 받을 수 있도록 했다.
